        ...
        <form action="" onSubmit={submit}>
          {imageDefaut && (
            <div className="w-full text-center relative">
              <img src={fileUpload} alt="" className="w-[150px] h-[150px] rounded-[50%]"></img>
              <label className="bg-primary-500 p-2 rounded-full absolute inline-flex text-white-500 bottom-3 right-[35%] cursor-pointer">
                <input className="hidden" type="file"  {...register('picture', { required: true, onChange: (e) => handleImage(e.target.files)})} />
                <FiEdit2 />
              </label>
            </div>
          )}
